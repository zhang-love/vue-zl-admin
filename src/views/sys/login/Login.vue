<template>
  <div :class="prefixCls" class="relative px-4">
    <AppLocalePicker class="absolute text-white top-4 right-4 enter-x xl:text-gray-600" />
    <AppDarkModeToggle class="absolute right-7 top-3 enter-x" />
    ...
    <div class="container relative h-full py-2 mx-auto sm:px-10">
      <div class="h-full flex">
        ...
        <div class="flex w-full h-full py-5 xl:h-auto xl:py-0 xl:my-0 xl:w-w-6/12">
          <div
            :class="`${prefixCls}-form`"
            class="relative w-full px-5 py-8 mx-auto my-auto rounded-md shadow-md xl:ml-16 xl:bg-transparent sm:px-8 xl:p-4 xl:shadow-none sm:w-3/4 lg:w-2/4 xl:w-auto -enter-x"
          >
            <LoginForm v-if="!formComp" />
            <component :is="formComp" v-else />
          </div>
        </div>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  watch(
    () => getLoginState.value,
    (val) => {
      formComp.value = createAsyncComponent(() => import(`../login/${val}.vue`));
    },
    {
      immediate: true,
    },
...
<style lang="less">
  ...
  @countdown-input-prefix-cls: ~'@{namespace}-countdown-input';
  @dark-bg: #293146;
  ...
  .@{prefix-cls} {
    ...
    .@{countdown-input-prefix-cls} input {
      min-width: unset;
    }
